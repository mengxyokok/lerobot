#!/usr/bin/env python
"""WebSocket服务器（MessagePack）：加载SAC模型并提供推理服务

使用方法:
1. 安装依赖: pip install websockets msgpack msgpack-numpy
2. 运行服务器: python sac_infer_remote_websocket_server_msgpack.py
    ...
    try:
        async for message in websocket:
            try:
                ...
                response = {"action": processed_action}
                response_data = msgpack.packb(response, default=m.encode)
                ...
                print(error_msg)
                try:
                    error_response = msgpack.packb({"error": str(e)}, default=m.encode)
...
async def main():
    async with websockets.serve(handle_client, "0.0.0.0", 5000):
        print("WebSocket服务器启动（MessagePack）: ws://0.0.0.0:5000")
